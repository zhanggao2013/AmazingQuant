...
class AShareCashFlow(Document):
    ...
    # 公告日期
    ann_dt = DateTimeField(required=True, null=True)
    # 报告期
    report_period = DateTimeField(required=True, null=True)
    # 报表类型
    ...
    # 实际公告日期
    actual_ann_dt = DateTimeField(required=True, null=True)
    # 经营活动现金流入差额(特殊报表科目)
...
class AShareProfitNotice(Document):
    ...
    security_code = StringField(required=True, null=True)
    # 公告日期
    s_profitnotice_date = DateTimeField(required=True, null=True)
    # 报告期
    s_profitnotice_period = DateTimeField(required=True, null=True)
    # 业绩预告类型代码
    ...
    # 首次公告日
    s_profitnotice_firstanndate = DateTimeField(required=True, null=True)
    # 业绩预告摘要
...
class AShareProfitExpress(Document):
    """
    A股业绩快报
    """
    # 更新时间
